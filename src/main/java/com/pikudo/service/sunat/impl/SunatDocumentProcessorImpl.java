...
import com.pikudo.config.properties.SunatProperties;
...
public class SunatDocumentProcessorImpl implements SunatDocumentProcessor {
    ...
    private void attachXml(SunatSubmissionJob job, java.util.UUID storageId) {
        ...
        if (job.getComprobante() != null) {
            job.getComprobante().setXmlStorageFile(com.pikudo.entity.storage.StorageFile.builder().id(storageId).build());
        } else if (job.getNotaCredito() != null) {
            job.getNotaCredito().setXmlStorageFile(com.pikudo.entity.storage.StorageFile.builder().id(storageId).build());
        } else if (job.getNotaDebito() != null) {
            job.getNotaDebito().setXmlStorageFile(com.pikudo.entity.storage.StorageFile.builder().id(storageId).build());
        }
    }
