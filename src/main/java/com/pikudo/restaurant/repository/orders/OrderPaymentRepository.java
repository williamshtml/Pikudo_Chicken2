...
public interface OrderPaymentRepository extends JpaRepository<OrderPayment, Long> {
    ...
    @Query("SELECT COALESCE(SUM(p.monto), 0) FROM OrderPayment p " +
           "WHERE p.caja.id = :cajaId AND p.status = com.pikudo.restaurant.entity.orders.OrderPaymentStatusType.CONFIRMED")
    BigDecimal sumConfirmedByCajaId(@Param("cajaId") Long cajaId);

    @Query("SELECT COALESCE(SUM(p.monto), 0) FROM OrderPayment p " +
           "WHERE p.caja.id = :cajaId AND p.metodoPago.tipo = :tipoMetodo " +
           "AND p.status = com.pikudo.restaurant.entity.orders.OrderPaymentStatusType.CONFIRMED")
    ...
    boolean existsByCajaIdAndPedidoEstadoPago(Long cajaId, com.pikudo.restaurant.entity.orders.OrderPaymentStatus estadoPago);
}
